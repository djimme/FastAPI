...
# 저장이 완료되면 외부로 노출돼도 되는 PublicUser 모델을 반환한다.
def create(user:PrivateUser, table:str="user") -> PublicUser:
    ...
    try:
        ...
        raise Duplicate(msg=f"user {user.name} already exists") from e
    return PublicUser(name=user.name)

# name이 외부로 노출되기 때문에 현재는 name에 대한 변경만 가능하다.
def modify(name:str, user:PublicUser) -> PublicUser:
    ...
    if curs.rowcount == 1:
        ...
        raise Missing(msg = f"User {name} not found for update")
...
def delete(name:str) -> None:
    ...
    curs.execute(qry, params)
    if curs.rowcount != 1:
        raise Missing(msg=f"user {name} not found for delete")
